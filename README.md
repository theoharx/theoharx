# Hi, I'm Hariharan S! 👋

## AI Engineer | Data Scientist | 3rd Year BTech Student | Problem Solver

I'm a 3rd-year B.Tech student in Artificial Intelligence and Data Science with a strong foundation in statistics, machine learning, deep learning, and big data analytics. As both an AI Engineer and Data Scientist, I'm passionate about building end-to-end data preprocessing pipelines, developing intelligent systems using advanced AI technologies like RAG and vector databases, and solving real-world problems at scale.

---

## 🎯 What I Do

- **AI Engineering**: Machine learning, deep learning, and core AI algorithms
- **Data Science & Analytics**: Big data processing, data visualization, and insights
- **Full-Stack ML**: From data preprocessing to model deployment and real-time predictions

---

## 💻 Tech Stack

### Programming & Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=java&logoColor=white)
![C](https://img.shields.io/badge/C-A8B9CC?style=flat&logo=c&logoColor=white)

### Tools & Technologies
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat&logo=streamlit&logoColor=white)

### Databases
![MySQL](https://img.shields.io/badge/MySQL-00758F?style=flat&logo=mysql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-13AA52?style=flat&logo=mongodb&logoColor=white)

### Cloud & DevOps
![AWS](https://img.shields.io/badge/AWS-FF9900?style=flat&logo=amazonaws&logoColor=white)
![DevOps](https://img.shields.io/badge/DevOps-0078D4?style=flat&logo=devops&logoColor=white)

### AI & Machine Learning
![RAG](https://img.shields.io/badge/RAG-FF6B6B?style=flat&logo=openai&logoColor=white)
![Vector DB](https://img.shields.io/badge/Vector%20DB-4B8BBE?style=flat&logoColor=white)
![Ollama](https://img.shields.io/badge/Ollama-000000?style=flat&logo=ollama&logoColor=white)
![LLM Pipelines](https://img.shields.io/badge/LLM%20Pipelines-50C878?style=flat&logoColor=white)

### Data Science & Analytics
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=flat&logo=scikitlearn&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?style=flat&logo=plotly&logoColor=white)

---

## 🚀 Featured Projects

### 🌾 **AgriTech Zen** – Precision Agriculture Ecosystem
*October 2025*
- Developed an AI-powered precision agriculture platform using deep learning and microservices
- Integrated satellite and drone data with IoT spectral images for comprehensive crop monitoring
- Features: Disease detection, water & soil stress analysis, yield prediction
- **Tech**: CNN, LSTM, Microservices, Real-time Analytics

### 🧠 **NeuroGuard AI** – Multimodal Alzheimer's Diagnostic Platform
*January 2026*
- Built an end-to-end medical AI application for early detection and risk assessment of Alzheimer's disease
- Ensemble model combining Random Forest and Gradient Boosting with Isotonic calibration
- Achieved high accuracy in early-stage disease identification
- **Tech**: Ensemble Learning, Calibration Techniques, Healthcare Analytics

### 📈 **Real-Time Stock Predictions**
*August 2025*
- Built a hybrid CNN-LSTM model for forecasting stock trends using candlestick patterns
- Integrated sentiment analysis for identifying market trends
- Real-time prediction capabilities with high accuracy
- **Tech**: CNN, LSTM, NLP, Time-Series Analysis

### 🏥 **Cirrhosis Detection System**
*October 2024*
- Developed an AI-driven diagnostic system using clinical data for liver cirrhosis detection
- Improved early-stage identification and reduced misdiagnosis rates
- Supports clinical decision-making with interpretable results
- **Tech**: Classification Algorithms, Healthcare ML, Data Preprocessing

---

## 📊 Skills & Expertise

| Category | Skills |
|----------|--------|
| **AI & LLMs** | RAG, Vector Databases, Ollama, LLM Pipelines, Prompt Engineering |
| **ML/DL** | CNN, LSTM, Ensemble Methods, Random Forest, Gradient Boosting, Stacking |
| **Data Science** | Scikit-learn, NumPy, Plotly, Data Visualization, Statistical Analysis |
| **Data Processing** | Data Cleaning, Feature Engineering, EDA, Preprocessing Pipelines |
| **Analytics** | Predictive Modeling, Time-Series Analysis, Healthcare Analytics |
| **Soft Skills** | Quick Learner, Problem Solving, Communication, Team Collaboration |

---

## 🎓 Education

**Bachelor of Technology** in Artificial Intelligence and Data Science  
*Dr. Mahalingam College of Engineering and Technology, Pollachi | Anna University*  
**Currently in 3rd Year** (2023-2027) | CGPA: 8.5 (Till 5th semester)

**HSC** | K.C.M Matric.Hr. Sec School, Nilakottai | 92.6%

---

## 📜 Certifications & Achievements

✅ **AWS Cloud Foundations Certification** – March 2026  
🏆 **1st Prize** – Pixel Hack Event at Dr MCET  
🥈 **2nd Prize** – Technical Quiz at KPR College

---

## 💼 Experience

### Data Analyst Intern | Elysium Technologies Private Limited, Madurai
*June 2025 – Present*
- Developing data analysis features using machine learning algorithms
- Hosting ML models and features using Streamlit for interactive analytics
- Collaborating with the team to improve data processing and reporting modules
- Optimizing data pipelines for better performance

---

## 📈 GitHub Stats

![Hariharan's GitHub Stats](https://github-readme-stats.vercel.app/api?username=theoharx&show_icons=true&theme=radical)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=theoharx&layout=compact&theme=radical)

---

## 🤝 Let's Connect!

[![Email](https://img.shields.io/badge/Email-inevitablehari@gmail.com-D14836?style=flat&logo=gmail&logoColor=white)](mailto:inevitablehari@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Hari-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/hari-3125h/)
[![GitHub](https://img.shields.io/badge/GitHub-theoharx-181717?style=flat&logo=github&logoColor=white)](https://github.com/theoharx)
[![Phone](https://img.shields.io/badge/Phone-%2B91%209962399767-25D366?style=flat&logo=whatsapp&logoColor=white)](tel:+919962399767)

---

## 🌟 What I'm Currently Doing

- 🤖 Building intelligent systems with RAG and vector databases
- 🧠 Exploring advanced LLM architectures and fine-tuning techniques
- 🔬 Developing deep learning models for healthcare and agriculture
- 📊 Creating data-driven insights using advanced analytics and visualization
- 💡 Contributing to open-source AI/ML projects
- 🌍 Solving real-world problems through end-to-end ML pipelines

---

### "Data tells stories. I help uncover them."

Let's collaborate and build something amazing together! Feel free to explore my repositories below. 👇

---

**Last Updated**: January 2026  
**Status**: Open to collaborations and opportunities in AI/ML and Data Science
